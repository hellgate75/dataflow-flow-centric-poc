...
$BASE_DIR/defaults.sh

source ./java_path.sh *> /dev/null
#export JAVA_HOME=
...
if [[ "$(beginswith "$(uname -a)" "CYGWIN")" == "Y" ]]; then
  CYGWIN=1
else
  CYGWIN=0
fi
...
#    KAFKA_GC_LOG_OPTS="-Xlog:gc*:file=$LOG_DIR/$GC_LOG_FILE_NAME:time,tags:filecount=10,filesize=102400"
    KAFKA_GC_LOG_OPTS="-Xloggc:$GC_LOG_FILE_NAME"
  else
...
(( CYGWIN )) && JAVA=$(cygpath --mixed "${JAVA}")

# Launch mode
if [ "x$DAEMON_MODE" = "xtrue" ]; then
  exec $JAVA $KAFKA_HEAP_OPTS $KAFKA_JVM_PERFORMANCE_OPTS $KAFKA_GC_LOG_OPTS $KAFKA_JMX_OPTS $KAFKA_LOG4J_OPTS -cp $CLASSPATH $KAFKA_OPTS "$@" > $CONSOLE_OUTPUT_FILE &#2> /dev/null &
  pid="$!"
  echo "$pid" > $LOG_DIR/$DAEMON_NAME.pid
  else
  #echo "exec $JAVA $KAFKA_HEAP_OPTS $KAFKA_JVM_PERFORMANCE_OPTS $KAFKA_GC_LOG_OPTS $KAFKA_JMX_OPTS $KAFKA_LOG4J_OPTS -cp $CLASSPATH $KAFKA_OPTS $@"
  sh -c "$JAVA $KAFKA_HEAP_OPTS $KAFKA_JVM_PERFORMANCE_OPTS $KAFKA_GC_LOG_OPTS $KAFKA_JMX_OPTS $KAFKA_LOG4J_OPTS -cp $CLASSPATH $KAFKA_OPTS $@"
fi
